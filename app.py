...
def get_flag(team_name):
    ...
    drapeaux = {
        ...
        "congo": "🇨🇩", "rd congo": "🇨🇩", "republique democratique du congo": "🇨🇩"
    }
...
if "matchs" not in st.session_state:
    st.session_state.matchs = {
        ...
        "16es - Match 5": {"team1": "Portugal", "flag1": "🇵🇹", "team2": "Croatie", "flag2": "🇭🇷", "score1_reel": 0, "score2_reel": 0, "qualifie_reel": "À Définir", "termine": False},
        "16es - Match 6": {"team1": "Espagne", "flag1": "🇪🇸", "team2": "Autriche", "flag2": "🇦🇹", "score1_reel": 0, "score2_reel": 0, "qualifie_reel": "À Définir", "termine": False},
        "16es - Match 7": {"team1": "USA", "flag1": "🇺🇸", "team2": "Bosnie", "flag2": "🇧🇦", "score1_reel": 0, "score2_reel": 0, "qualifie_reel": "À Définir", "termine": False},
        "16es - Match 8": {"team1": "Belgique", "flag1": "🇧🇪", "team2": "Sénégal", "flag2": "🇸🇳", "score1_reel": 0, "score2_reel": 0, "qualifie_reel": "À Définir", "termine": False},
        ...
        "16es - Match 12": {"team1": "Angleterre", "flag1": "🏴󠁧󠁢󠁥󠁮󠁧󠁿", "team2": "RD Congo", "flag2": "🇨🇩", "score1_reel": 0, "score2_reel": 0, "qualifie_reel": "À Définir", "termine": False},
        ...
        "16es - Match 15": {"team1": "Suisse", "flag1": "🇨🇭", "team2": "Algérie", "flag2": "🇩🇿", "score1_reel": 0, "score2_reel": 0, "qualifie_reel": "À Définir", "termine": False},
        "16es - Match 16": {"team1": "Colombie", "flag1": "🇨🇴", "team2": "Ghana", "flag2": "🇬🇭", "score1_reel": 0, "score2_reel": 0, "qualifie_reel": "À Définir", "termine": False},
...
# --- RESULTADO 1N2 (vitória equipa 1 / empate / vitória equipa 2) ---
def _resultado_1n2(s1, s2):
    if s1 > s2: return "1"
    if s1 < s2: return "2"
    return "X"

# Pontos de um único prognóstico já validado, face ao resultado real de um jogo.
# Regras: 1pt resultado certo (1N2) aos 90' · 3pts resultado exato (substitui o 1pt,
# não acumula) · +1pt SEMPRE que a equipa qualificada prevista esteja certa (este soma-se
# sempre, independentemente do resultado).
def calcular_pontos_jogo(prono, info):
    pts = 0
    s1, s2 = prono.get("score1"), prono.get("score2")
    r1, r2 = info["score1_reel"], info["score2_reel"]
    if s1 == r1 and s2 == r2:
        pts += 3
    elif _resultado_1n2(s1, s2) == _resultado_1n2(r1, r2):
        pts += 1
    if prono.get("qualifie") == info["qualifie_reel"]:
        pts += 1
    return pts
        ...
        for match_id, info in st.session_state.matchs.items():
            if info["termine"] and match_id in user_pronos:
                ...
                if isinstance(prono, dict) and prono.get("valide", False):
                    total_points += calcular_pontos_jogo(prono, info)
        ...
        for match_id, info in st.session_state.matchs.items():
            ...
            if info["termine"]:
                ...
                if isinstance(prono, dict) and prono.get("valide"):
                    pts_jogo = calcular_pontos_jogo(prono, info)
                    fim_txt = f"Fim: {info['score1_reel']}-{info['score2_reel']}"
                    if pts_jogo >= 3:
                        st.markdown(f"<span style='float:right;' class='badge-status'>🟢 EXATO (+{pts_jogo}) | {fim_txt}</span>", unsafe_allow_html=True)
                    elif pts_jogo > 0:
                        st.markdown(f"<span style='float:right;' class='badge-status'>🟡 +{pts_jogo} PT{'S' if pts_jogo > 1 else ''} | {fim_txt}</span>", unsafe_allow_html=True)
                    else:
                        st.markdown(f"<span style='float:right;' class='badge-status'>🔴 FALHADO | {fim_txt}</span>", unsafe_allow_html=True)
                else:
                    st.markdown(f"<span style='float:right;' class='badge-status'>⚪ SEM APOSTA | Fim: {info['score1_reel']}-{info['score2_reel']}</span>", unsafe_allow_html=True)
            elif deja_valide:
